package com.example.administrator.suvsproject.modules.MyLogin.net;
...
//            }
            requestParams.setAsJsonContent(true);//传输的参数类型为json//和requestParams.addHeader("Content-Type","application/json")这句效果是一样的;
            String json="{\"content\":\"发现界面能不能做的好点一点都不好用！\",\"title\":\"13875268652\"}";
            requestParams.setBodyContent(json);//将json作为body写入到服务器
        }
...
//                        String json="{\"content\":\"发现界面能不能做的好点一点都不好用！\",\"title\":\"13875268652\"}";
    ...
    public static void parseJson() throws JSONException {
        String data="{data:{info:{result:{list:[1,2,3,4,5,6]}},info1:name}}";
        JSONObject jsonObject=new JSONObject(data);
        JSONObject dataJson=jsonObject.getJSONObject("data");
        JSONObject infoJson=dataJson.getJSONObject("info");
        String name=dataJson.getString("name");
        JSONObject resultJson= infoJson.getJSONObject("result");
        JSONArray list=resultJson.getJSONArray("list");
        for (int i = 0; i < list.length(); i++) {
            String number=list.getString(i);
        }
    }
}
